...
- **Executive Reports and Data Export**: Dedicated administrative export center for generating custom User and Room activity reports:
  - **Export Formats**: Multi-format exports in CSV, XLS, and XLSX.
  - **Spreadsheet Branding**: Centered brand logo, full-width executive metadata card, and center-aligned numeric/status data tables.
...
| `/api/admin/reports/users/` | `GET` | User activity data preview and file export (`?format=csv\|xls\|xlsx`) |
| `/api/admin/reports/rooms/` | `GET` | Room activity data preview and file export (`?format=csv\|xls\|xlsx`) |
