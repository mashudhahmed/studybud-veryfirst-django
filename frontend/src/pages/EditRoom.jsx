...
const EditRoom = () => {
  ...
  const fetchRoomData = async () => {
    try {
      ...
      if (user && room.host && user.username !== room.host.username) {
        navigate('/');
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const room = await updateRoom(id, formData);
  ...
  if (loading && !formData.name) {
    return (
      <div style={{ padding: '48px', textAlign: 'center', color: '#7a7c90' }}>
        Loading...
      </div>
    );
  }
